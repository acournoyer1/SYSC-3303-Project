...
public class Client
{
	 ...
	 * Used for checking TID in client
	 * 
	 * @param error number
		...
		while(index == -1) {
			...
			else if((receivePacket.getData()[3] == 1 || receivePacket.getData()[3] == 2) && receivePacket.getData()[1] == 5)
			{
				...
				if(receivePacket.getPort() != hostTID){
					if(verbose)
						System.out.println("Received ACK from port:" + receivePacket.getPort() + " when expecting port:" + hostTID);
					//If the ports do not match, send an errorpacket to the received packet
					createSendError(new Byte("5"), receivePacket, socket);
